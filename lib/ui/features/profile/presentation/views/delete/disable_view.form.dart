...
final Map<String, TextEditingController> _DisableViewTextEditingControllers =
    {};

final Map<String, FocusNode> _DisableViewFocusNodes = {};

final Map<String, String? Function(String?)?> _DisableViewTextValidations = {
  ReasonValueKey: null,
  OthersValueKey: null,
};
...
mixin $DisableView {
  ...
  TextEditingController _getFormTextEditingController(
    ...
    if (_DisableViewTextEditingControllers.containsKey(key)) {
      return _DisableViewTextEditingControllers[key]!;
    }

    _DisableViewTextEditingControllers[key] =
        TextEditingController(text: initialValue);
    return _DisableViewTextEditingControllers[key]!;
  }

  FocusNode _getFormFocusNode(String key) {
    if (_DisableViewFocusNodes.containsKey(key)) {
      return _DisableViewFocusNodes[key]!;
    }
    _DisableViewFocusNodes[key] = FocusNode();
    return _DisableViewFocusNodes[key]!;
  }
  ...
  void disposeForm() {
    ...
    for (var controller in _DisableViewTextEditingControllers.values) {
      controller.dispose();
    }
    for (var focusNode in _DisableViewFocusNodes.values) {
      focusNode.dispose();
    }

    _DisableViewTextEditingControllers.clear();
    _DisableViewFocusNodes.clear();
  }
}
...
extension ValueProperties on FormStateHelper {
  ...
  set reasonValue(String? value) {
    ...
    if (_DisableViewTextEditingControllers.containsKey(ReasonValueKey)) {
      _DisableViewTextEditingControllers[ReasonValueKey]?.text = value ?? '';
    }
  }
  ...
  set othersValue(String? value) {
    ...
    if (_DisableViewTextEditingControllers.containsKey(OthersValueKey)) {
      _DisableViewTextEditingControllers[OthersValueKey]?.text = value ?? '';
    }
  }
...
String? getValidationMessage(String key) {
  final validatorForKey = _DisableViewTextValidations[key];
  if (validatorForKey == null) return null;

  String? validationMessageForKey = validatorForKey(
    _DisableViewTextEditingControllers[key]!.text,
  );
